...
Last updated: M7, ingester durability — and the first CI run.

M7 cleared the two entries that stood between the gateway and a customer's
pipeline, and they turned out to be one piece of work. **D10** — dedupe as a
bounded in-memory window — and **D15** — heartbeats that only cover chains the
process has already seen — meet at a restart: seeding writers from the store
fixes the second and immediately triggers the first, because a re-seeded writer
has no memory of the heartbeat it already wrote today.

Both failures have the shape this project exists to prevent. Neither errors,
neither gaps, and the chain verifies perfectly through both. Running
`chaos-gate` with the durable check disabled prints `[PASS] chain verifies with
no gap` directly above `[FAIL] every event recovered — 100502 of 100001`: the
record is internally consistent and describes five hundred requests that never
happened. That is why the fix is a lookup before a `seq` is allocated rather
than a repair afterwards — once the seq is spent there is nothing to repair,
and the checkpointer stops signing that chain for good.

What is left is listed below, and none of it blocks an install.

**D5 is cleared: CI has run.** The repository has a remote, the pipeline has
executed, and all seven jobs are green — so the claims here, including
`chaos-gate`'s four new checks, are no longer claims about one 20-core dev
machine. The first run is worth recording, because it failed in none of the
ways D5 predicted. `latency-gate` and `quickstart` were the two jobs the entry
expected to be fragile on a shared runner; both passed, `quickstart` in 280s
against a 45-minute budget, and the resolver's headroom turned out to be wide
enough (p99 171ns against a 5µs target) that four cores absorb what twenty did.
What failed instead was everything no machine but this one had ever executed.
...
- ~~D10 dedupe is a bounded in-memory window~~ — the window stays, because it
  answers most redeliveries without a round trip; what it grew is a durable
  backstop. `EventStore::stored` asks the store which of a batch's `event_id`s
  it already holds, **before** anything is chained, and the answer is folded
  into the writer's window so `append` refuses them exactly as it refuses one
  this process wrote itself. The previous entry said the fix was "a unique
  index on `event_id`", and that was wrong in a way worth recording: ClickHouse
  has no unique constraint, and `ReplacingMergeTree` deduplicates in the
  background, long after the damage — an allocated `seq` — is done. So the
  enforcement is a lookup and the index is only what makes it cheap
  (`INDEX idx_event_id … TYPE bloom_filter`, added to `001` and to `002` for
  tables that already exist). A store that cannot answer defers the batch
  rather than guessing, on the same reasoning as `head`
- ~~D15 heartbeats only cover chains this process has already seen~~ —
  `Ingester::seed_from_store` takes over every chain in the store at startup,
  so the chains whose silence the heartbeat exists to make countable are
  covered from the first tick rather than never. Not fatal if the store is
  unreachable at boot: the ingester logs it loudly and comes up, because
  traffic builds its own writers and refusing to start would trade a missing
  heartbeat for a missing gateway. See D11 — this makes the writer count a
  tenant-count problem at startup rather than after a busy day
- the fast path is preserved and asserted, not assumed. A writer whose chain
  started empty and whose window has never evicted *is* a complete record, so
  the durable check is skipped entirely — which is every chain in a new
  deployment. `a_redelivery_the_window_cannot_remember_is_caught_by_the_store`
  asserts the lookup count is zero before the restart and non-zero after,
  because a check that quietly stopped running would look exactly like one that
  was never needed
...
The M5 and M4 lists are deleted; they are in the git history and the entries
they cleared have not come back.
...
| D11 | **One `ChainWriter` per chain, unbounded.** An ingester serving ten thousand systems holds ten thousand writers, each with a dedupe window. No eviction. The gateway's drop counter has the same shape and *is* bounded — 4 096 chains, then an `unknown/unknown` overflow bucket — which is the pattern to copy here. **M7 made this arrive sooner:** `seed_from_store` builds a writer for every chain in the store at startup rather than waiting for traffic, so the count is now the tenant count from the first second rather than after a busy day | `ancre-ingester/src/pipeline.rs` | Memory grows with tenant count rather than with traffic, and now does so at startup |
